...
export const actions = {
  ...
  async getProductByCategoryId({commit}, [id, page]){
    const products = await this.$axios.$get(`http://apidavmar.neoteric-software.com/api/product/getByCategoryId/${id}?page=${page}`);
    commit('setProductByCategory', products)
  },
  async getCategoryFilters({commit}, [id]){
    const brandFilters = await this.$axios.$get(`http://apidavmar.neoteric-software.com/api/product/getCategoryFilters/${id}`);
    commit('setCategoryFilters', brandFilters)
  },
  async Filter({commit}, [filter, priceRange, brand, page = 0]){
    const filteredProducts = await this.$axios.$post(`http://apidavmar.neoteric-software.com/api/product/filter`, {'filters': filter, 'priceRange': priceRange, 'brand': brand, 'page': page});
    commit('setProductByBrand', filteredProducts)
  },
  async FilterByCategory({commit}, [filter, priceRange, category, page = 0]){
    const filteredProducts = await this.$axios.$post(`http://apidavmar.neoteric-software.com/api/product/filterByCategory`, {'filters': filter, 'priceRange': priceRange, 'category': category, 'page': page});
    commit('setProductByCategory', filteredProducts)
  },
  async getProductsByIds({commit}, [ids]){
    const ProductsByIds = await this.$axios.$post(`http://apidavmar.neoteric-software.com/api/product/getProductsByIds`, {'ids': ids});
  ...
  async salesProducts({commit}){
    const filterAsTypeProduct = await this.$axios.$get(`http://apidavmar.neoteric-software.com/api/product/fiterAsSalesType`);
  ...
  async updateProduct({commit}, [id, name_en, name_ru, name_am, category, price, selectedImages, selectedColors,  selectedSizes, selectedBrand, sex, isNew, discountType, discount, description_en, description_ru, description_am]){
    await this.$axios.$put(`http://apidavmar.neoteric-software.com/api/product/update/${id}`, {'name_en': name_en, 'name_ru': name_ru, 'name_am': name_am, 'category': category, 'price': price, 'selectedImages': selectedImages, 'selectedColors': selectedColors, 'sizes': selectedSizes, 'selectedBrand': selectedBrand, 'sex': sex, 'isNew': isNew, 'discountType': discountType, 'discount': discount, 'description_en': description_en, 'description_ru': description_ru, 'description_am': description_am});
  },
  ...
  async addProduct(ctx, [name_en, name_ru, name_am, category, price, selectedImages, selectedColors,  selectedSizes, selectedBrand, sex, isNew, discountType, discount, description_en, description_ru, description_am])  {
    await this.$axios.$post('http://apidavmar.neoteric-software.com/api/product/add', {'name_en': name_en, 'name_ru': name_ru, 'name_am': name_am, 'category': category, 'price': price, 'selectedImages': selectedImages, 'selectedColors': selectedColors, 'sizes': selectedSizes, 'selectedBrand': selectedBrand, 'sex': sex, 'isNew': isNew, 'discountType': discountType, 'discount': discount, 'description_en': description_en, 'description_ru': description_ru, 'description_am': description_am});
  }
}

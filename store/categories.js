...
export const actions = {
  ...
  async updateCategory({commit}, [id, name_en, name_ru, name_am, order, image, color, selectedBrand, parentCategory]){
    await this.$axios.$put(`http://apidavmar.neoteric-software.com/api/category/update/${id}`, {'name_en': name_en, 'name_ru': name_ru, 'name_am':name_am, 'order': order, 'image': image, 'color': color, 'selectedBrand': selectedBrand, 'parentCategory': parentCategory});
  },
  ...
  async addCategory(ctx, [name_en, name_ru, name_am, order, image, color, selectedBrand, parentCategory]) {
    await this.$axios.$post('http://apidavmar.neoteric-software.com/api/category/add', {'name_en': name_en, 'name_ru': name_ru, 'name_am':name_am, 'order': order, 'image': image, 'color': color, 'selectedBrand': selectedBrand, 'parentCategory': parentCategory});
  }
};
